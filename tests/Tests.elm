    ...
    [ describe "Parse elements from geojson"
      [ test "can parse geometry from GeoJson" <|
        ...
        ,test "can parse feature collection from GeoJson" <|
          \() ->
            let
              featureObject = { geometry = Just (Point(1,3,[])), properties = Json.Encode.string "", id = Nothing }
              testGeoJson = ( FeatureCollection( [ featureObject ] ) , Nothing )
            in
              Expect.equal (parseFeatureCollection testGeoJson) (Just [ featureObject ])
        ...
        ,test "can parse polygon coordinates from geometry with a single polygon" <|
          \() ->
            ...
            in
              Expect.equal (parsePolygonCoordinates geometry) (Just [ [ coordinates1, coordinates2 ] ] )
        ...
        ,test "can parse polygon coordinates from geometry with multiple polygons" <|
          \() ->
            ...
            in
              Expect.equal (parsePolygonCoordinates geometry) (Just [
                [ coordinates1 ]
                , [ coordinates2 ]
                , [ coordinates3 ]
                ])
        ...
        ,test "can generate a single-polygon string for rendering from GeoJson" <|
          \() ->
            let
              ...
              featureObject = { geometry = Just (geometry), properties = Json.Encode.string "", id = Nothing }
              testGeoJson = ( FeatureCollection( [ featureObject ] ) , Nothing)
            in
              Expect.equal (generatePolygonStrings testGeoJson) ["10.45,8.33 22.85,93.11 74.29,123.09"]
        ...
        ,test "can generate a multi-polygon strings for rendering from GeoJson" <|
          \() ->
            let
              ...
              testGeoJson = ( FeatureCollection( [ featureObject ] ) , Nothing)
            in
              Expect.equal (generatePolygonStrings testGeoJson) ["10.45,8.33 22.85,93.11", "74.29,123.09 34.2,171.89"]
      ]
    ]
